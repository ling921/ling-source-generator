using LingDev.EntityFrameworkCore.Audit.Generator;
...
public sealed partial class AuditGenerator
{
    private sealed class Parser
    {
        ...
        private const string IHasCreationTimeFullName = "LingDev.Audit.IHasCreationTime";
        private const string IHasCreatorFullName = "LingDev.Audit.IHasCreator`1";
        private const string IHasModificationTimeFullName = "LingDev.Audit.IHasModificationTime";
        private const string IHasModifierFullName = "LingDev.Audit.IHasModifier`1";
        private const string ISoftDeleteFullName = "LingDev.Audit.ISoftDelete";
        private const string IHasDeletionTimeFullName = "LingDev.Audit.IHasDeletionTime";
        private const string IHasDeleterFullName = "LingDev.Audit.IHasDeleter`1";
            ...
            foreach (IGrouping<SyntaxTree, ClassDeclarationSyntax> group in classDeclarationSyntaxList.GroupBy(c => c.SyntaxTree))
            {
                ...
                foreach (var classDeclarationSyntax in group)
                {
                    ...
                    contextGenSpecList.Add(new ContextGenerationSpec()
                    {
                        ...
                        {
                            Location = contextLocation,
                            ClassDeclaration = classDeclarationList.First()!,
