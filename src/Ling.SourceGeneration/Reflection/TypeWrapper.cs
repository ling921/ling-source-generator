...
namespace LingDev.EntityFrameworkCore.SourceGeneration.Reflection;
...
internal class TypeWrapper : Type
{
    ...
    public override string FullName
    {
        get
        {
            if (_fullName == null)
            {
                ...
                {
                    sb.Append(GetElementType().FullName).Append("[]");
                }
                else
                {
                    sb.Append(Name);

                    for (var currentSymbol = _typeSymbol.ContainingSymbol; currentSymbol != null && currentSymbol.Kind != SymbolKind.Namespace; currentSymbol = currentSymbol.ContainingSymbol)
                    {
                    ...
                    if (IsGenericType && !ContainsGenericParameters)
                    {
                        ...
                        foreach (var genericArg in GetGenericArguments())
                        {
                            sb.Append("[");
    ...
    {
        foreach (var constructor in GetConstructors(bindingAttr))
        {
    ...
    protected override PropertyInfo GetPropertyImpl(string name, BindingFlags bindingAttr, Binder binder, Type returnType, Type[] types, ParameterModifier[] modifiers)
    {
        ...
        foreach (var propertyInfo in GetProperties(bindingAttr))
        {
    ...
    public override Type[] GetInterfaces()
    {
        ...
        foreach (var i in _typeSymbol.AllInterfaces)
        {
    ...
    public override MethodInfo[] GetMethods(BindingFlags bindingAttr)
    {
        ...
        {
            // TODO: Efficiency
            if (m is IMethodSymbol method && _namedTypeSymbol?.Constructors.Contains(method) == false)
            {
    ...
    public override bool IsAssignableFrom(Type c)
    {
        if (c is TypeWrapper tr)
        {
            return tr._typeSymbol.AllInterfaces.Contains(_typeSymbol, SymbolEqualityComparer.Default) ||
                (tr._namedTypeSymbol?.BaseTypes().Contains(_typeSymbol, SymbolEqualityComparer.Default) == true);
        }
        else if (_metadataLoadContext.Resolve(c) is TypeWrapper trr)
        {
            return trr._typeSymbol.AllInterfaces.Contains(_typeSymbol, SymbolEqualityComparer.Default) ||
                (trr._namedTypeSymbol?.BaseTypes().Contains(_typeSymbol, SymbolEqualityComparer.Default) == true);
        }
        return false;
    }
